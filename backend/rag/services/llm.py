...
def embed_texts(texts: list[str]) -> list[list[float]]:
    ...
    try:
        response = client.embeddings.create(model=settings.EMBEDDING_MODEL, input=texts, **kwargs)
    ...
    if len(vectors) != len(texts) or any(len(v) != EMBEDDING_DIMENSIONS for v in vectors):
        raise LLMUnavailable(
            f"Embedding model {settings.EMBEDDING_MODEL!r} returned an unexpected shape; "
            f"expected {len(texts)} vectors of size {EMBEDDING_DIMENSIONS}."
        )
    return vectors
